...
最終更新日：2021年7月11日の時点guide version 4.2.1
{% endhint %}
...
{% hint style="info" %}
IOHK発表の最小構成はメモリ8GBですが、最近では間に合わなくなってきています。
ご予算の都合もあるかと思いますが、メモリ16GB以上での構成を推奨します。
{% endhint %}
...
* **メモリー:** 8GB RAM（サードパーティー製ツールを導入する場合4GBのスワップ領域を設定するか、10GB以上のメモリを搭載）
...
{% hint style="info" %}
\*\*\*\*✨ **ヒント:** ホットキーを作成したら、コールドキーへのアクセス権限を変更しセキュリティを向上させることができます。これによって誤削除、誤った編集などから保護できます。
...
2つの送金方法があります。
{% hint style="info" %}
**1.payment.addrへ送金する方法**は[こちら](guide-how-to-build-a-haskell-stakepool-node.md#18-9-1-paymentaddrhesuru)

**2.任意のアドレスへ送金する方法は**[こちら](guide-how-to-build-a-haskell-stakepool-node.md#18-9-2-noadoresuhesuru)
{% endhint %}
